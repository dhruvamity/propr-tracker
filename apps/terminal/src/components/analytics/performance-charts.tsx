      ...
      <div className="p-4 rounded-lg border border-[var(--border-primary)] bg-[var(--bg-surface)] space-y-3 font-sans">
        ...
        <div className="h-32 flex items-end gap-2 pt-4 border-b border-zinc-800">
          {durationBuckets.map((bucket, idx) => {
            ...
            return (
              <div key={idx} className="flex-1 flex flex-col items-center justify-end h-full group relative">
                <div className="absolute -top-5 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none text-[11px] font-mono text-zinc-300">
                  {bucket.count}
                </div>
                <div
                  ...
                  style={{ height: `${hPct}%` }}
                />
                <span className="text-[11px] text-zinc-400 mt-1 truncate">{bucket.range}</span>
              </div>
            );
          })}
        </div>
      ...
      <div className="p-4 rounded-lg border border-[var(--border-primary)] bg-[var(--bg-surface)] space-y-3 font-sans">
        ...
        <div className="h-32 flex items-end gap-2 pt-4 border-b border-zinc-800">
          {weekdayPnl.map((day, idx) => {
            ...
            return (
              <div key={idx} className="flex-1 flex flex-col items-center justify-end h-full group relative">
                <div className="absolute -top-5 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none text-[11px] font-mono text-zinc-300 whitespace-nowrap">
                ...
                <div
                  ...
                  style={{ height: `${hPct}%` }}
                />
                <span className="text-[11px] text-zinc-400 mt-1 truncate">{day.day}</span>
              </div>
            );
          })}
        </div>
